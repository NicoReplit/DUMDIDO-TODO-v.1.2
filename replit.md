...
- **Device Preview Mode**: A development feature that constrains the app view to specific device resolutions for testing. Primary device is Custom Display (800x1280 portrait). Toggle with Ctrl+D (Cmd+D on Mac). Includes device selector and scale options (50%, 75%, 100%). The DevicePreview component injects CSS custom properties (--device-width, --device-height, --device-vmin) for device-aware sizing of decorative elements.
- **Fixed Layout Design**: The app uses a fixed layout optimized for the 800x1280 portrait display. Only the todo cards within the grey rectangle area scroll, while the header, user selector, progress bar, week calendar, and decorative bottom characters remain fixed. Decorative elements (RedMenu, BlueMenu, CelebrationMenu) use CSS custom properties for positioning to work correctly in both device preview and fullscreen modes.
